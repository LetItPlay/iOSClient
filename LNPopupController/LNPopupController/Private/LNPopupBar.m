...
const CGFloat LNPopupBarHeightProminent = 72.0;
const CGFloat LNPopupBarProminentImageWidth = 40.0;
...
{
	return UIBlurEffectStyleProminent; //systemBarStyle == UIBarStyleBlack ? UIBlurEffectStyleDark : barStyle == LNPopupBarStyleCompact ? UIBlurEffectStyleExtraLight : UIBlurEffectStyleLight;
}
...
//		_progressView.trackImage = [UIImage alloc];
		_progressView.trackTintColor = [[UIColor alloc] initWithRed:243.0/255 green:71.0/255 blue:36.0/255 alpha:0.1];
		_progressView.progressTintColor = [[UIColor redColor] colorWithAlphaComponent:0.2];
		[_toolbar addSubview:_progressView];
		[self _updateProgressViewWithStyle:LNPopupBarProgressViewStyleBottom];
		...
		_marqueeScrollEnabled = YES;//[NSProcessInfo processInfo].operatingSystemVersion.majorVersion < 10;
...
- (void)_updateProgressViewWithStyle:(LNPopupBarProgressViewStyle)style
{
	...
	{
        _progressViewVerticalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[_progressView(2)]-6-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_progressView)];
    }
...
- (void)_layoutTitles
{
	dispatch_async(dispatch_get_main_queue(), ^{
		...
		if(_needsLabelsLayout == YES)
		{
			...
			NSMutableDictionary* defaultTitleAttribures = [@{NSParagraphStyleAttributeName: paragraph, NSFontAttributeName: _resolvedStyle == LNPopupBarStyleProminent ? [UIFont systemFontOfSize:14 weight:UIFontWeightBold] : [UIFont systemFontOfSize:12]} mutableCopy];
			[defaultTitleAttribures addEntriesFromDictionary:_titleTextAttributes];
			
			NSMutableDictionary* defaultSubtitleAttribures = [@{NSParagraphStyleAttributeName: paragraph, NSFontAttributeName: [UIFont systemFontOfSize:14 weight:UIFontWeightRegular]} mutableCopy];
		...
		titleLabelFrame.origin.y = _imageView.frame.origin.y + _imageView.frame.size.height - _titleLabel.font.lineHeight + 3;
		titleLabelFrame.size.height = _titleLabel.font.lineHeight;
...
//		titleLabelFrame.size.height = barHeight;
		if(_subtitle.length > 0)
		{
			...
			subtitleLabelFrame.origin.y = _imageView.frame.origin.y + 4;
//			subtitleLabelFrame.size.height = barHeight;
			subtitleLabelFrame.size.height = _subtitleLabel.font.lineHeight;
			
//			if(_resolvedStyle == LNPopupBarStyleProminent)
//			{
//				subtitleLabelFrame.origin.y -= _titleLabel.font.lineHeight / 2.1;
//				titleLabelFrame.origin.y += _subtitleLabel.font.lineHeight / 1.5;
//			}
//			else
//			{
//				subtitleLabelFrame.origin.y -= _titleLabel.font.lineHeight / 2;
//				titleLabelFrame.origin.y += _subtitleLabel.font.lineHeight / 2;
//			}
...
- (void)_layoutImageView
{
	...
	_imageView.center = CGPointMake(safeLeading + LNPopupBarProminentImageWidth / 2, LNPopupBarHeightProminent / 2 - 4);
